...
export async function searchQuestions(query: string, limit: number = 5): Promise<(Question & { answerCount: number; authors: Array<{id: string, name: string, photo?: string, professional_identity?: string}> })[]> {
  ...
  const similarities = questions.map((q: any) => {
    ...
    const authorProfessionalIdentities = q.authorProfessionalIdentities ? q.authorProfessionalIdentities.split(',') : [];
    
    const authors = authorIds.map((id: string, index: number) => ({
      id: id,
      name: authorNames[index] || '',
      photo: authorPhotos[index] !== 'null' && authorPhotos[index] ? authorPhotos[index] : undefined,
      professional_identity: authorProfessionalIdentities[index] !== 'null' && authorProfessionalIdentities[index] ? authorProfessionalIdentities[index] : undefined
    }));
...
export function getAnswersForQuestion(questionId: string): (Answer & { authorName: string; authorPhoto?: string; authorProfessionalIdentity?: string })[] {
  const database = initDatabase();
  const stmt = database.prepare(`
...
export function getAnswersByAuthor(authorId: string): (Answer & { question: string })[] {
  const database = initDatabase();
  const stmt = database.prepare(`
    SELECT a.*, q.question 
    FROM answers a 
...
export function hasAnswers(questionId: string): boolean {
  const database = initDatabase();
  const stmt = database.prepare('SELECT COUNT(*) as count FROM answers WHERE question_id = ?');
  const result = stmt.get(questionId) as { count: number };
  return result.count > 0;
}

export function getQuestionsWithAnswersPaginated(page: number = 1, limit: number = 12): {
  questions: (Question & { answerCount: number; authors: Array<{id: string, name: string, photo?: string, professional_identity?: string}> })[];
  totalCount: number;
  hasMore: boolean;
  currentPage: number;
  totalPages: number;
  ...
  return {
    questions: questions.map((question: any) => {
      ...
      const authorNames = question.authorNames ? question.authorNames.split(',') : [];
      const authorPhotos = question.authorPhotos ? question.authorPhotos.split(',') : [];
      const authorProfessionalIdentities = question.authorProfessionalIdentities ? question.authorProfessionalIdentities.split(',') : [];
...
export function getQuestionsWithoutAnswersPaginated(page: number = 1, limit: number = 12): PaginatedQuestions {
  const database = initDatabase();
...
export function getTotalCounts() {
  ...
  // Get total questions
  const totalQuestionsStmt = database.prepare('SELECT COUNT(*) as count FROM questions');
  const totalQuestions = (totalQuestionsStmt.get() as { count: number }).count;
  
  // Get total voices
  const totalProPalestiniansStmt = database.prepare('SELECT COUNT(*) as count FROM voices');
  const totalProPalestinians = (totalProPalestiniansStmt.get() as { count: number }).count;
  
  // Get total answers
  const totalAnswersStmt = database.prepare('SELECT COUNT(*) as count FROM answers');
  ...
  const questionsWithAnswersStmt = database.prepare(`
    ...
    JOIN answers a ON q.id = a.question_id
  `);
  const questionsWithAnswers = (questionsWithAnswersStmt.get() as { count: number }).count;
...
export function getQuestionsWithMostAnswers(limit: number = 6): (Question & { answerCount: number; authors: Array<{id: string, name: string, photo?: string, professional_identity?: string}> })[] {
  const database = initDatabase();
  const stmt = database.prepare(`
    SELECT 
      q.*,
