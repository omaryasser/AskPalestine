...
export async function loadDataToDatabase() {
  ...
  if (fs.existsSync(proPalestiniansPath)) {
    ...
    for (const proPalestinianDir of proPalestinianDirs) {
      const proPalestinianPath = path.join(proPalestiniansPath, proPalestinianDir);
      ...
      if (fs.existsSync(metadataPath)) {
        try {
          ...
          console.warn(`Error reading metadata for ${proPalestinianDir}:`, error);
        }
      }
      ...
      for (const ext of photoExtensions) {
        ...
        if (fs.existsSync(photoPath)) {
          ...
          const publicPhotoPath = path.join(publicPhotosDir, `${proPalestinianDir}${ext}`);
      ...
      insertProPalestinian.run(proPalestinianDir, proPalestinianDir, bio, photo, professional_identity);
    }
  }
  ...
  for (const questionDir of questionDirs) {
    ...
    insertQuestion.run(questionDir, questionText, JSON.stringify(embedding), JSON.stringify(metadata));
    ...
    if (fs.existsSync(answersPath)) {
      ...
      for (const answerDir of answerDirs) {
        ...
        if (fs.existsSync(answerMetadataPath)) {
          answerMetadata = JSON.parse(fs.readFileSync(answerMetadataPath, 'utf-8'));
        }
